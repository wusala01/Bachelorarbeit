...
\paragraph{} As an example (cf. \Cref{fig:ucEx}): An Automated Teller Machine (ATM) is used by the customer with the goal of performing an ATM Transaction, which can specify in either a money transfer, withdraw or deposit. For transfer and withdraw, a authentication via card is required. If the user runs into trouble, ATM Transactions all have a extension point (which in this case most likely would be a button) to receive help. This is regarded as a extension of the goal, as the user does not have to make demands on getting helped.
...
\begin{figure}[H] 
    ...
    \begin{minipage}[m]{.5\linewidth}
        ...
        \subcaption{Example}\label{fig:adEx}
    \end{minipage}
